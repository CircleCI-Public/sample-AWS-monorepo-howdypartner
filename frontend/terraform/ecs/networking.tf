...
resource "aws_security_group" "aws-app-22" {
  description = "Port 22 Admin"
  vpc_id      = aws_vpc.vpc.id
  name_prefix = "aws-app-22-SSH-"
...
resource "aws_security_group" "aws-app-ELB" {
  description = "Port 443 Elastic Load Balancer"
  name_prefix = "aws-app-443-ELB-"
  vpc_id      = aws_vpc.vpc.id
  ...
  tags = {
    Name  = "aws-app-443-ELB",
    team  = "DevRel Marketing",
    owner = "Angel Rivera"
  }
}

resource "aws_security_group" "aws-app-ELB-HTTP80" {
  description = "Port 80 Elastic Load Balancer"
  name_prefix = "aws-app-ELB-80-"
  vpc_id      = aws_vpc.vpc.id
...
resource "aws_security_group" "aws-app-80" {
  description = "Port 80 internal dev group "
  vpc_id      = aws_vpc.vpc.id
  name_prefix = "aws-app-80-internal-"
