...
data "template_file" "user_data" {
  ...
  vars = {
    CLUSTER_NAME = aws_ecs_cluster.aws-app.name
  }
}
...
resource "aws_launch_configuration" "app" {
  name_prefix = "aws-app-"
  security_groups = [
    aws_security_group.aws-app-22.id,
    aws_security_group.aws-app-80.id,
    aws_security_group.aws-app-ELB-HTTP80.id
  ]
...
# AWS SNS Topic
resource "aws_sns_topic" "aws-app-sns" {
  name         = "aws-app-notifications"
  display_name = "aws-app scaling notifications"
}
...
resource "aws_autoscaling_notification" "asg_notifications" {
  ...
  topic_arn = aws_sns_topic.aws-app-sns.arn
  depends_on = [
    aws_autoscaling_group.app
  ]
}
...
resource "aws_cloudwatch_log_group" "awslogs-aws-app" {
  name = "awslogs-aws-app"
  tags = {
...
resource "aws_ecs_task_definition" "aws-app" {
  family                = "aws-app"
  container_definitions = data.template_file.task_definition_json.rendered
}

#AWS ECS Cluster
resource "aws_ecs_cluster" "aws-app" {
  name = "aws-app"
}

#AWS ECS Service
resource "aws_ecs_service" "aws-app" {
  name                               = "srv_aws-app"
  cluster                            = aws_ecs_cluster.aws-app.name
  desired_count                      = 2
  deployment_minimum_healthy_percent = 50
  deployment_maximum_percent         = 100
  task_definition                    = aws_ecs_task_definition.aws-app.arn
  load_balancer {
    target_group_arn = aws_alb_target_group.alb.id
